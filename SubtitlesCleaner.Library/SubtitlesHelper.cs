...
namespace SubtitlesCleaner.Library
{
    public static class SubtitlesHelper
    {
        ...
        {
            return CleanSubtitle(subtitle, cleanHICaseInsensitive, isDictionaryCleaning, false, isPrintCleaning);
        }
        ...
        {
            return CleanSubtitles(subtitles, cleanHICaseInsensitive, isDictionaryCleaning, false, isPrintCleaning);
        }

        private static Subtitle CleanSubtitle(this Subtitle subtitle, bool cleanHICaseInsensitive, bool isDictionaryCleaning, bool isCheckMode, bool isPrintCleaning)
        {
            return new List<Subtitle>() { subtitle }.CleanSubtitles(cleanHICaseInsensitive, isDictionaryCleaning, isCheckMode, isPrintCleaning)?.FirstOrDefault();
        }

        private static List<Subtitle> CleanSubtitles(this List<Subtitle> subtitles, bool cleanHICaseInsensitive, bool isDictionaryCleaning, bool isCheckMode, bool isPrintCleaning)
        {
            if (subtitles.IsNullOrEmpty())
                return null;
            ...
            subtitles = IterateSubtitlesPost(subtitles, cleanHICaseInsensitive, isDictionaryCleaning, isCheckMode, isPrintCleaning);
            if (subtitles.IsNullOrEmpty())
                return null;
        ...
        private static List<Subtitle> IterateSubtitlesPost(List<Subtitle> subtitles, bool cleanHICaseInsensitive, bool isDictionaryCleaning, bool isCheckMode, bool isPrintCleaning)
        {
            if (subtitles.IsNullOrEmpty())
                return null;
            ...
            for (int k = subtitles.Count - 1; k >= 0; k--)
            {
                ...
                if (subtitle.Lines.HasAny())
                {
                    subtitleError = SubtitleError.None;
                    List<string> cleanLines = subtitle.Lines.GetRange(0, subtitle.Lines.Count);
                    cleanLines = CleanSubtitleMultipleLinesPost(cleanLines, cleanHICaseInsensitive, isDictionaryCleaning, isCheckMode, ref subtitleError, isPrintCleaning);
                    bool isChanged =
                        cleanLines == null ||
                        subtitle.Lines.Count != cleanLines.Count ||
                        subtitle.Lines.Zip(cleanLines, (l1, l2) => l1 != l2).Any(isLineChanged => isLineChanged);
            ...
            if (lines.Count == 1)
            {
                ...
                if (line.StartsWith("- ") == false)
                {
                        ...
                        (index2 = line.IndexOf("</i> - ")) != -1)
                    {
                        ...
                        lines.Clear();
                        lines.Add("- " + line.Substring(0, index1 + prefixLength));
                        lines.Add(line.Substring(index1 + prefixLength + 1));
        ...
        private static List<string> CleanSubtitleMultipleLinesPost(List<string> lines, bool cleanHICaseInsensitive, bool isDictionaryCleaning, bool isCheckMode, ref SubtitleError subtitleError, bool isPrintCleaning)
        {
            if (lines.IsNullOrEmpty())
            {
                if (isCheckMode)
        ...
        public static readonly FindAndReplace[] NonSubtitle = new FindAndReplace[] {
            ...
            ,new FindAndReplace(new Regex(@"(?i)ENGLISH PSDH", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)ENGLISH SDH", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)English-Subtitles", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ...
            ,new FindAndReplace(new Regex(@"(?i)isubtitles", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)MKV Player", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Movie subtitles", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Moviesubtitles", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ...
            ,new FindAndReplace(new Regex(@"(?i)SharePirate", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)strong language and violent scenes", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Subdl", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Subs by", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Subs4free", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Subscene", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Subtitle Seeker", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ...
            ,new FindAndReplace(new Regex(@"(?i)Subtitles Hub", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Subtitles:", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)SubtitlesHub", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Subtitletools", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Subtitling", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Sync & corrections", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)Sync and corrections", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ...
            ,new FindAndReplace(new Regex(@"(?i)The following (?:programme|program) contains strong language", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ...
            ,new FindAndReplace(new Regex(@"(?i)TV subs", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)TV subtitles", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)TVsubs", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
            ,new FindAndReplace(new Regex(@"(?i)TVsubtitles", RegexOptions.Compiled), "", SubtitleError.Non_Subtitle)
        ...
        public static readonly FindAndReplace[] HearingImpaired = new FindAndReplace[] {
            ...
            // Start Sentence</i> MAN: Text => Start Sentence</i> - Text
            ,new FindAndReplace("HI Without Dialog",
                                new Regex(@"(?<Prefix>[.?!]|</i>)\s*[A-ZÀ-Ý0-9 #\'\[\]][A-ZÀ-Ý0-9 #\-'\[\]]*[A-ZÀ-Ý#'\[\]][A-ZÀ-Ý0-9 #\-'\[\]]*:\s*(?<Subtitle>.+?)$", RegexOptions.Compiled), "${Prefix} - ${Subtitle}", SubtitleError.Hearing_Impaired)
                    .SetRegexCI(new Regex(@"(?<Prefix>[.?!]|</i>)\s*[A-ZÀ-Ýa-zà-ÿ0-9 #\'\[\]][A-ZÀ-Ýa-zà-ÿ0-9 #\-'\[\]]*[A-ZÀ-Ýa-zà-ÿ#'\[\]][A-ZÀ-Ýa-zà-ÿ0-9 #\-'\[\]]*:(?!\d\d)\s*(?<Subtitle>.+?)$", RegexOptions.Compiled))
            ...
            // Start Sentence</i> MAN (laughting): Text => Start Sentence</i> - Text
            ,new FindAndReplace("HI Without Dialog",
                                new Regex(@"(?<Prefix>[.?!]|</i>)\s*[" + HI_CHARS.Replace("-'", "'") + @"]+\(.*?\):\s*", RegexOptions.Compiled), "${Prefix} - ", SubtitleError.Hearing_Impaired)
                    .SetRegexCI(new Regex(@"(?<Prefix>[.?!]|</i>)\s*[" + HI_CHARS_CI.Replace("-'", "'") + @"]+\(.*?\):(?!\d\d)\s*", RegexOptions.Compiled))
            ,new FindAndReplace("HI Without Dialog",
                                new Regex(@"(?<Prefix>[.?!]|</i>)\s*[" + HI_CHARS.Replace("-'", "'") + @"]+\[.*?\]:\s*", RegexOptions.Compiled), "${Prefix} - ", SubtitleError.Hearing_Impaired)
                    .SetRegexCI(new Regex(@"(?<Prefix>[.?!]|</i>)\s*[" + HI_CHARS_CI.Replace("-'", "'") + @"]+\[.*?\]:(?!\d\d)\s*", RegexOptions.Compiled))
        ...
        public static readonly FindAndReplace[] MissingSpaces = new FindAndReplace[] {
            ...
            // Text...'Text' => Text... 'Text'
            // 'Text...'$ => ignore
            // (?!$) is negative lookahead, prevents capturing pattern '$ at the end of a line
        ...
        public static readonly FindAndReplace[] I_And_L = new FindAndReplace[] {
            ...
            ,new FindAndReplace(new Regex(@"\b[IVXLCDM]*(?<OCR>l)[IVX]*\b", RegexOptions.Compiled), "OCR", "I", SubtitleError.I_And_L_Error,
                ...
                , new FindAndReplace.IgnoreRule() { ReadNextCharsFromMatch = 5, IgnoreIfEqualsTo = "l... l" }
